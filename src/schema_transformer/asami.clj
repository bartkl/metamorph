(ns schema-transformer.asami
  (:require [asami.core :as d] [clojure.java.io :as io]
...
(def model
  (->>
   (rdf/read-directory (io/file "resources/example-profile/")
                       (fn [path] (into-array IRI [(utils/iri-from-filename path)])))
   (map #((juxt :rdf/subject :rdf/predicate :rdf/object) %))))

(take 10 model)

@(d/transact conn {:tx-triples model})

(d/q '[:find ?x :where [_ _ ?x]] conn)
...
(as->
 (d/q '[:find [?r ...] ;;?label ?comment
        :where
        [?r "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" _]
        ...
        [?r "http://www.w3.org/ns/shacl#property"* (first ?p)]
...
(d/entity conn "https://w3id.org/schematransform/ExampleShape#BShape" true)
(d/entity conn "https://w3id.org/schematransform/ExampleShape#idShape" true)
